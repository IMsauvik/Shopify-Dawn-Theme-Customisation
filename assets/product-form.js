...
onSubmitHandler(evt){evt.preventDefault();if(this.submitButton.getAttribute('aria-disabled')==='true')return;

const variantId = this.form.querySelector('[name=id]').value;
const isOfferProduct = this.checkIfOfferProduct(variantId);
if (isOfferProduct && !this.isFromCartDrawer()) {
  this.handleErrorMessage('This gift is claimable only from the cart drawer after reaching ₹500+');
...
checkIfOfferProduct(variantId) {
  const cartProgressBar = window.cartProgressBarInstance;
  if (cartProgressBar && cartProgressBar.availableOffers) {
    return cartProgressBar.availableOffers.some(product => 
      product.variants && product.variants.some(variant => variant.id.toString() === variantId.toString())
    );
  }
  return false;
}
isFromCartDrawer() {
  return this.closest('cart-drawer') !== null || 
         document.querySelector('cart-drawer.active') !== null;
}
