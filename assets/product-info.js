    ...
    class ProductInfo extends HTMLElement {
      ...
      handleUpdateProductInfo(productUrl) {
        return (html) => {
          ...
          this.handleBreezeVariant(variant);
      ...
      handleBreezeVariant(variant) {
        try {
          const breezeButton =
            this.parentElement?.getElementsByTagName("breeze-button")[0];
          breezeButton?.setAttribute(
            "selectedproductvariantdata",
            btoa(JSON.stringify(variant))
          );
          if (!variant?.available) {
            console.log("Unavailable", variant);
